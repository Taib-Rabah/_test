...
export const isInClosedRange = (min: number, value: number, max: number): boolean => value >= min && value <= max;
...
export const minmax = (min: number, value: number, max: number): number => Math.min(Math.max(min, value), max);
